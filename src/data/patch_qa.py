"""Generate supplementary visual QA for the frozen Stage 0 patch dataset.

This command is intentionally separate from patch extraction.  It reads the
frozen manifests and source DICOMs, reconstructs only a deterministic review
sample, and writes review artefacts without changing any patch or manifest.
Run it on the CUDA data host, where the source DICOM tree is available.
"""
...
class PatchQAConfig:
    """Deterministic controls for the supplementary visual review."""
...
@dataclass(frozen=True)
class ReviewSource:
    """One preprocessed mammogram and its aligned known-ROI masks."""
...
def validate_reconstructed_case(
    ...
    """Prove that one frozen patch and its recorded mask geometry reconstruct."""
...
def _review_template(groups: list[str]) -> str:
    checklist = "\n".join(f"- [ ] `{group}` reviewed" for group in groups)
    return f"""# Stage 0 supplementary QA review template

This generated template is part of the locked QA package. Copy it to
`review-decision.md` before recording the human decision; do not edit this file.

For every grid, verify that the cyan patch box, green target ROI and magenta union
of known ROIs agree in both local source context and the extracted patch. Review
bright structures, tissue boundaries and dark regions in both background grids.

{checklist}

## Decision

- [ ] Patch/ROI alignment accepted
- [ ] Reviewed backgrounds show no annotated-lesion contamination
- [ ] Fallback cases remain suitable for the registered patch task
- [ ] Stage A training authorised
...
def _summary(
    ...
    return {
        ...
        "stage_a_training_authorised": False,
        "config": {
            "seed": config.seed,
        ...
        "manual_review_required": [
            "patch and ROI alignment in source context",
            "fallback lesion suitability",
            "background bright structures and tissue-edge cases",
            "absence of annotated-lesion contamination in reviewed backgrounds",
        ],
    }
...
def generate_patch_qa(
    ...
    with tempfile.TemporaryDirectory(
        ...
        template_path = temporary_dir / "review-template.md"
        template_path.write_text(_review_template(groups))
