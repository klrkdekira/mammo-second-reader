...
def freeze_evidence(
    ...
    *,
    ...
    runs = data["runs"]
    missing = [run.get("model", "<unnamed>") for run in runs if "provenance" not in run]
    if missing:
        raise ValueError(f"Run records are missing provenance: {missing}")
    old_provenance = [
        run.get("model", "<unnamed>")
        for run in runs
        if run["provenance"].get("version", 0) < MIN_PROVENANCE_VERSION
    ]
    if old_provenance:
        raise ValueError(
            "Run records do not include case-level prediction provenance: "
            f"{old_provenance}"
        )
    ...
    preprocessing = {
        run["provenance"]["code"]["preprocessing_fingerprint"] for run in runs
    }
    evaluation = {run["provenance"]["code"]["evaluation_fingerprint"] for run in runs}
    dirty = [
        run["model"]
        for run in runs
        if run["provenance"]["git"].get(
            "dirty_evidence_files",
            run["provenance"]["git"].get("dirty_tracked_files", False),
        )
    ]
    ...
    if len(preprocessing) != 1 or len(evaluation) != 1:
        raise ValueError(
            "Runs were generated by different preprocessing/evaluation code."
        )
    if dirty and not allow_dirty:
        raise ValueError(
            f"Runs were generated from dirty evidence worktrees: {dirty}. "
    ...
    for run in runs:
        ...
        _verify_descriptor(provenance["config"], root)
        for descriptor in provenance["checkpoints"]:
            _verify_descriptor(descriptor, root)
        for descriptor in provenance["manifests"]:
            _verify_descriptor(descriptor, root)
        threshold = provenance.get("threshold_sidecar")
        if threshold is not None:
            _verify_descriptor(threshold, root)
        for descriptor in provenance["code"]["preprocessing_files"]:
            _verify_descriptor(descriptor, root)
        for descriptor in provenance["code"]["evaluation_files"]:
            _verify_descriptor(descriptor, root)
        predictions = provenance.get("prediction_files", [])
